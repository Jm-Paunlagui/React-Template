...
export function Select({
    ...
    const handleMultiToggle = (optValue) => {
        ...
        const next = selectedValues.includes(str)
            ? selectedValues.filter((v) => v !== str)
            : [...selectedValues, str];
        onChange?.(next);
    };
                ...
                <li
                    ...
                    className={`flex items-center justify-between cursor-pointer font-aumovio-bold
                        ...
                        ${isActive
                            ...
                            : "text-black/70 dark:text-white/70 hover:bg-orange-400/10 hover:text-orange-400"
                        }`}
                >
                    <span>{o.label}</span>
    ...
    return (
        <div className="font-aumovio">
            {label && (
                <label
                    htmlFor={inputId}
                    className="block text-xs font-aumovio-bold text-black/70 dark:text-white/70 mb-1.5 cursor-default"
                >
                    {label}
                </label>
            )}
            ...
            <div ref={ref} className="relative">
                ...
                >
                    <span className={`truncate text-sm ${!hasValue ? "text-black/40 dark:text-white/40" : "font-aumovio-bold"}`}>
                ...
                {open && (() => {
                    const dropdownEl = (
                        <ul
                            ...
                            style={
                                fixed
                                    ? { position: "fixed", top: fixedPos.top, left: fixedPos.left, width: fixedPos.width, scrollbarWidth: "none" }
                                    : { scrollbarWidth: "none" }
                            }
                        >
                            ...
                            {groups.length > 0 ? (
                                <>
                                    ...
                                    {groups.map((g) => (
                                        <li key={g}>
                                            <div className="px-3 pt-2 pb-0.5 text-xs font-aumovio-bold text-black/40 dark:text-white/40 uppercase tracking-wider select-none">
                                                {g}
                                            </div>
            ...
            {error && (
                <p className="mt-1.5 text-xs text-danger-400 font-aumovio-bold">{error}</p>
            )}
        </div>
    );
}
