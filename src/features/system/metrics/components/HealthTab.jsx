...
export default function HealthTab({ hook }) {
  ...
  const fetchHealth = async () => {
    ...
    try {
      const [liveRes, readyRes] = await Promise.allSettled([
        httpClient.get('health/live'),
        httpClient.get('health/ready'),
      ]);
      setLiveness(
        liveRes.status === 'fulfilled' ? liveRes.value.data : null,
      );
      setReadiness(
        readyRes.status === 'fulfilled'
          ? readyRes.value.data
          : readyRes.reason?.response?.data ?? null,
      );
    } catch {
  ...
  return (
    <div className={`mt-6 space-y-4 ${ANIMATE_ENTER_UP} ${ANIM_DELAY_0}`}>
      ...
      {/* Liveness */}
      <div className={`p-5 rounded-2xl border ${ANIMATE_ENTER_UP} ${staggerDelay(0)} ${
        liveOk
          ? 'bg-white dark:bg-[#1a1030] border-grey-100 dark:border-white/10'
          : 'bg-danger-400/5 border-danger-400/30 dark:border-danger-400/40'
      }`}>
        <div className="flex items-center gap-3 mb-3">
          <Badge variant={liveOk ? 'green' : 'red'}>
            {liveOk ? 'Live' : 'Down'}
          </Badge>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          <div>
            ...
            <p className={`font-medium ${liveOk ? 'text-success-400' : 'text-danger-400'}`}>
              {liveData.pid ?? '—'}
            </p>
          </div>
          <div>
            ...
            <p className={`font-medium ${liveOk ? 'text-success-400' : 'text-danger-400'}`}>
              {liveData.uptime ? `${Math.round(liveData.uptime)}s` : '—'}
            </p>
          </div>
          <div>
            <p className="text-xs text-grey-500 dark:text-grey-400 uppercase tracking-wide">Checked at</p>
            <p className={`font-medium text-xs ${liveOk ? 'text-success-400' : 'text-danger-400'}`}>
      ...
      {/* Readiness */}
      <div className={`p-5 rounded-2xl border ${ANIMATE_ENTER_UP} ${staggerDelay(1)} ${
        readyOk
          ? 'bg-white dark:bg-[#1a1030] border-grey-100 dark:border-white/10'
          : 'bg-danger-400/5 border-danger-400/30 dark:border-danger-400/40'
      }`}>
        <div className="flex items-center gap-3 mb-4">
          <Badge variant={readyOk ? 'green' : 'red'}>
            ...
            {Object.entries(checks).map(([name, check]) => (
              <div
                ...
                className={`flex items-center justify-between px-3 py-2.5 rounded-lg border ${
                  check.status === 'up'
                    ? 'bg-grey-50 dark:bg-white/5 border-grey-100 dark:border-white/10'
                    : 'bg-danger-400/10 border-danger-400/30'
                }`}
              >
                <span className={`font-mono text-xs ${check.status === 'up' ? 'text-grey-700 dark:text-grey-300' : 'text-danger-400'}`}>
                  {name}
                </span>
                <div className="flex items-center gap-3">
                  {check.latencyMs != null && (
                    <span className={`${PILL_BASE} ${getHealthLatencyStyle(check.latencyMs)}`}>
                      {check.latencyMs}ms
                    </span>
                  )}
                  <Badge
                    variant={check.status === 'up' ? 'green' : 'red'}
                    size="sm"
                  >
